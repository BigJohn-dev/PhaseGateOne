...
function getValidResponse(questionText, questionNum) {
	while (true) {
		console.log(questionText);
		const response = prompt(`Please select (A or B): `).toUpperCase();
...
function calculateMBTI(responses) {
	...
	for (const [traitPair, indexes] of Object.entries(traits)) {
		let aCount = 0, bCount = 0;
		for (const i of indexes) {
			if (responses[i] === 'A') aCount++;
			else bCount++;
}
		result.push(aCount >= bCount ? traitPair[0] : traitPair[1]);
}
	return result.join('');
}
...
People with the ENTJ personality type (Commanders) are natural-born leaders. Embodying the gifts of charisma and confidence, ENTJs project authority in a way that draws crowds together behind a common goal. However, these personalities are also characterized by an often ruthless level of rationality, using their drive, determination, and sharp mind to achieve whatever objectives they’ve set for themselves. Their intensity might sometimes rub people the wrong way, but ultimately, ENTJs take pride in both their work ethic and their impressive level of self-discipline.
    ...
    return personalities[personalityType] || "Unknown personality type.";
}

function MBTImain() {
	console.log("Welcome to the MBTI Personality Test!");
	console.log("Please answer each question by selecting A or B.\n");
	...
	let totalB = 0;
 	...
 	for (let i = 0; i < questions.length; i++) {
		const answer = getValidResponse(questions[i], i + 1);
		...
		if (answer === 'A') totalA++;
		 ...
		 if ((i + 1) % 5 === 0) {
            		console.log(`The number of A selected: ${totalA}`);
			console.log(`The number of B selected: ${totalB}\n`);

			totalA = 0;
			totalB = 0;
}
}
	const mbtiType = calculateMBTI(responses);

	console.log(`\nYour MBTI Personality Type: ${mbtiType}`);
	console.log(`\nDescription:\n${getPersonalityDescription(mbtiType)}`);
}
