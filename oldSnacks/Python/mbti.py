...
def calculate_mbti(answers):
	return ''.join(
		pair[0] if sum(answers[i] == 'A' for i in idx) >= 3 else pair[1]
		for pair, idx in trait_groups.items()
	)
...
def main():
	...
	for i, q in enumerate(questions):
		...
		if (i + 1) % 5 == 0:
			group = answers[i - 4: i + 1]
			print(f"The number of A selected: {group.count('A')}")
			print(f"The number of B selected: {group.count('B')}\n")

	mbti = calculate_mbti(answers)
	print(f"\nYour MBTI Type is: {mbti}")
	print("\n" + mbti_personalities.get(mbti, "No personality description found."))
